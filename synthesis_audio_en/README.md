...
* Read the paragraphs of text from your GCS bucket, folder `text_paragraphs`
* Use the **Google Cloud Text-to-Speech API** to generate an audio file in English
* Save the audio file in your bucket, folder `text_audios` (same file name, audio extension)
...
`cd` into the `synthesis_audio_en` folder (you already have it from cloning the [mega-pipeline](https://github.com/dlops-io/mega-pipeline/tree/flexible-workflow) repo).
...
<your-workspace>/
├── mega-pipeline/
│   └── synthesis_audio_en/   ← you are here
└── secrets/
    └── mega-pipeline.json
```
...
> The image is named `mega-pipeline-synthesis-audio-en`. To publish under your own DockerHub org rather than `dlops`, change `DOCKER_USERNAME` at the top of `docker-shell.sh`.
...
python cli.py -s   # synthesize English audio (written directly to your bucket)
```

Then verify in the GCP console that `text_audios/<group_name>/` in your bucket contains the new audio files.
